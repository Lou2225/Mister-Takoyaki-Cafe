<header id="top-bar" class="bg-white border-b border-gray-200 sticky top-0 z-[100] flex items-center justify-between px-4 py-3 sm:px-6">
        ...
        <div class="relative" x-data="{ notificationsOpen: false }" @close-notifications.window="notificationsOpen = false" wire:poll.30s>
            ...
            <div x-show="notificationsOpen" x-transition x-cloak
                ...
                <div class="px-4 py-3 border-b border-gray-50 flex items-center justify-between bg-gray-50/30">
                    ...
                    @if($unreadCount > 0)
                        <button wire:click="markAllAsRead" class="text-[10px] font-bold text-indigo-600 hover:text-indigo-800 transition-colors">Mark All as Read</button>
                    @endif
                </div>
                <div style="max-height: 320px !important; overflow-y: auto !important;">
                    @forelse($notifications as $n)
                        <div class="relative group/n">
                            <button wire:click="traceNotification({{ $n['id'] }})" 
