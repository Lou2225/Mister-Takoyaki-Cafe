                        ...
                        <template x-for="(item, key) in cart" :key="key">
                            <div class="flex items-start gap-2.5 py-3">
                                ...
                                <div class="w-11 h-11 rounded-lg overflow-hidden bg-gray-100 shrink-0 border border-gray-100">
                                    <img :src="item.image ? (item.image.startsWith('http') ? item.image : '{{ asset('storage') }}/' + item.image.replace(/^\/?storage\//, '')) : '{{ asset('images/placeholder-product.png') }}'" onerror="this.onerror=null;this.src='{{ asset('images/placeholder-product.png') }}';" class="w-full h-full object-cover" :alt="item.name">
                                </div>
