<div 
                        ...
                        <div class="w-full aspect-square rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/50 flex items-center justify-center relative overflow-hidden transition-all group-hover:border-indigo-300 group-hover:bg-indigo-50/30">
                            ...
                            @else
                                <div class="text-center p-6">
                                    <div class="w-12 h-12 rounded-2xl bg-white border border-slate-100 flex items-center justify-center text-slate-300 mx-auto mb-3 shadow-sm group-hover:scale-110 transition-transform">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                    </div>
                                    <span class="text-[11px] font-black text-slate-400 uppercase tracking-widest leading-tight block">Upload Image</span>
                                </div>
                            @endif
    ...
    <div x-show="panel === 'list'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" x-cloak class="px-1">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div class="bg-gradient-to-br from-indigo-50 to-indigo-100 border border-indigo-200 rounded-2xl p-4 shadow-sm flex items-center gap-4 group hover:shadow-md transition-all">
                ...
                <div>
                    ...
                    <span class="block text-[20px] font-black text-gray-900 leading-none">{{ number_format($totalProducts) }}</span>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200 rounded-2xl p-4 shadow-sm flex items-center gap-4 group hover:shadow-md transition-all">
                ...
                <div>
                    <span class="block text-[10px] font-black text-emerald-700/60 uppercase tracking-widest leading-none mb-1">Active Items</span>
                    <span class="block text-[20px] font-black text-emerald-600 leading-none">{{ number_format($this->activeCount) }}</span>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-rose-50 to-rose-100 border border-rose-200 rounded-2xl p-4 shadow-sm flex items-center gap-4 group hover:shadow-md transition-all">
                ...
                <div>
                    <span class="block text-[10px] font-black text-rose-700/60 uppercase tracking-widest leading-none mb-1">Hidden Items</span>
                    <span class="block text-[20px] font-black text-rose-600 leading-none">{{ number_format($this->hiddenCount) }}</span>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-amber-50 to-amber-100 border border-amber-200 rounded-2xl p-4 shadow-sm flex items-center gap-4 group hover:shadow-md transition-all">
                ...
                <div>
                    <span class="block text-[10px] font-black text-amber-700/60 uppercase tracking-widest leading-none mb-1">Categories</span>
                    <span class="block text-[20px] font-black text-gray-900 leading-none">{{ number_format($totalCategories) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="relative z-20 flex flex-col lg:flex-row lg:items-center justify-between mb-6 gap-4 bg-white p-2.5 rounded-xl border border-slate-200/60 shadow-sm">
            ...
            <div class="flex flex-1 w-full lg:w-auto">
                <x-search-bar wireModel="search" placeholder="Find product..." width="w-full lg:w-72" />
            </div>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <x-secondary-button type="button" class="gap-1.5 h-9 !px-3 bg-white hover:bg-slate-50 border-slate-200 text-slate-600 shadow-none">
                            ...
                            <span class="text-[12px] font-semibold whitespace-nowrap">{{ $isActive === '1' ? 'Active Only' : ($isActive === '0' ? 'Hidden Only' : 'All Status') }}</span>
                            <svg class="w-3 h-3 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7" /></svg>
                        </x-secondary-button>
                    </x-slot>
                    <x-slot name="content">
    ...
    <x-modal name="confirm-save-product" maxWidth="sm" focusable wire:key="modal-confirm-save">
        ...
        <div class="p-6">
            <div class="flex items-start gap-4">
                <div class="w-12 h-12 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-600 shrink-0 shadow-sm border border-emerald-100">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                </div>
                <div>
                    <h3 class="text-[16px] font-black text-slate-900 leading-tight">Sync to Catalog?</h3>
                    <p class="text-[13px] text-slate-500 mt-1.5 leading-relaxed font-medium">This will finalize all changes and update the product across all active branch menus.</p>
                </div>
            </div>
            <div class="mt-8 flex flex-col gap-2">
                <x-primary-button wire:click="saveProduct" class="w-full justify-center h-12 text-[13px] font-black uppercase tracking-widest bg-emerald-600 hover:bg-emerald-700 shadow-lg shadow-emerald-100">
                    {{ $editProductId ? 'Update Product' : 'Register Product' }}
                </x-primary-button>
                <x-secondary-button @click="$dispatch('close-modal', 'confirm-save-product')" class="w-full justify-center h-11 text-[12px] font-black uppercase tracking-widest border-slate-200">
                    Review Again
                </x-secondary-button>
            </div>
        </div>
    ...
    <x-modal name="delete-product" maxWidth="sm" focusable wire:key="modal-delete-product">
        ...
        <div class="p-6">
            <div class="flex items-start gap-4">
                <div class="w-12 h-12 rounded-2xl bg-rose-50 flex items-center justify-center text-rose-600 shrink-0 shadow-sm border border-rose-100">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                </div>
                <div>
                    <h3 class="text-[16px] font-black text-slate-900 leading-tight">Purge Product?</h3>
                    <p class="text-[13px] text-slate-500 mt-1.5 leading-relaxed font-medium">Are you sure you want to remove <span class="text-slate-900 font-bold">"{{ $deleteTargetName }}"</span>? All associated recipes and branch configurations will be lost permanently.</p>
                </div>
            </div>
            <div class="mt-8 flex items-center justify-end gap-3">
                <x-secondary-button @click="$dispatch('close-modal', 'delete-product')" class="h-11 px-6 text-[12px] font-black uppercase tracking-widest border-slate-200">Cancel</x-secondary-button>
                <x-danger-button wire:click="deleteProduct" @click="$dispatch('close-modal', 'delete-product')" class="h-11 px-6 text-[12px] font-black uppercase tracking-widest bg-rose-600 shadow-lg shadow-rose-100">Confirm Deletion</x-danger-button>
            </div>
        </div>
    </x-modal>
    ...
    <x-modal name="quick-add-category" maxWidth="sm" focusable wire:key="modal-quick-add-cat">
        ...
        <div class="p-6">
            <h3 class="text-[16px] font-black text-slate-900 uppercase tracking-widest">New Category Entry</h3>
            <p class="text-[12px] text-slate-400 font-medium mt-1">Add a new category to organize your menu.</p>
            
            <div class="mt-6">
                <x-input-label value="Category Name" />
                <x-text-input wire:model.debounce.400ms="newCategoryName" class="w-full mt-2 h-11" placeholder="e.g. Snacks, Beverages, Desserts" inputFilter="name" :hasError="$errors->has('newCategoryName')" />
                <x-input-error :messages="$errors->get('newCategoryName')" class="mt-1" />
            </div>
            
            <div class="flex items-center justify-end gap-3 mt-8">
                <x-secondary-button @click="$dispatch('close-modal', 'quick-add-category')" class="h-11 px-6 text-[12px] font-black uppercase tracking-widest border-slate-200">Cancel</x-secondary-button>
                <x-primary-button wire:click="quickAddCategory()" class="h-11 px-6 text-[12px] font-black uppercase tracking-widest bg-indigo-600 shadow-lg shadow-indigo-100">Create Category</x-primary-button>
            </div>
        </div>
    </x-modal>
    ...
    <x-modal name="add-option-group" maxWidth="sm" focusable wire:key="modal-add-option-group">
        <div class="h-1.5 w-full bg-indigo-600"></div>
        <div class="p-6">
            <h3 class="text-[16px] font-black text-slate-900 uppercase tracking-widest">Configure Option Group</h3>
            <p class="text-[12px] text-slate-400 font-medium mt-1">Groups allow customers to customize their order.</p>
            ...
            <div class="mt-6 space-y-6">
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <x-dropdown align="left" width="full" containerClasses="block w-full">
                            <x-slot name="trigger">
                                <button type="button" class="flex items-center justify-between w-full px-4 py-2 bg-white border border-slate-200 rounded-xl text-[14px] text-slate-700 shadow-sm hover:border-slate-300 focus:outline-none transition-all h-11">
                ...
                <div class="flex items-center gap-3 p-4 bg-slate-50/50 rounded-2xl border border-slate-100 transition-all hover:bg-indigo-50/30">
                    ...
                    <label for="is_req_group" class="text-[12px] font-bold text-slate-700 cursor-pointer">Mandatory Selection (Customer must choose)</label>
                </div>
            </div>
            <div class="flex items-center justify-end gap-3 mt-8">
                <x-secondary-button @click="$dispatch('close-modal', 'add-option-group')" class="h-11 px-6 text-[12px] font-black uppercase tracking-widest border-slate-200">Cancel</x-secondary-button>
                <x-primary-button wire:click="addOptionGroup" class="h-11 px-6 text-[12px] font-black uppercase tracking-widest bg-indigo-600 shadow-lg shadow-indigo-100">Add Group</x-primary-button>
            </div>
        </div>
    </x-modal>
</div>
