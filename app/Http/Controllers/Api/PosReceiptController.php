    ...
    public function data(Order $order): JsonResponse
    {
        ...
        return response()->json([
            'order' => [
                ...
                'claim_number'     => $order->table_number ?? $order->reference_no,
                'cashier_name'     => $order->user ? trim(($order->user->first_name ?? '') . ' ' . ($order->user->last_name ?? '')) : 'STAFF',
                'account_id'       => $order->user?->employee_id ?? ('ACC-' . str_pad($order->user_id ?? 0, 4, '0', STR_PAD_LEFT)),
